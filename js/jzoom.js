...
jQuery.fn.gzoom = function(settings) {
	...
	return this.each(function(){
	  ...
	  var swapped = false;
		...
		var $slider = jQuery('<div id="sliderzoom"></div>').insertAfter($div).css({ width:(settings.sW-42)+'px'});
      ...
      if(settings.debug && typeof(console) != 'undefined') {
        console.log('Img dimensions:'+imgWidth+'x'+imgHeight);
			...
			var intWidth = (imgWidth + (settings.containerBorderSize * 2)); // Plus the image??s width and the left and right padding value
			var intHeight = (imgHeight + (settings.containerBorderSize * 2)); // Plus the image??s height and the left and right padding value
